...
## date: '2020-12-22'
## This R file is designed for cleaning the 2017 General Social Survey (GSS17) data
## from the 2019 Canadian Election Study. The "Workspace set-up" part is the pre-work
## done by Rohan Alexander and Sam Caetano (see Preamble section).
...
setwd("/Users/xhaic/Desktop/UofT/STA304/FP/raw data")
raw_data <- read.csv("AASwqbWi.csv")
...
labels_raw_tibble <- as_tibble(str_split(labels_raw, ";")[[1]]) %>% 
  ...
  dplyr::select(variable_name, cases) %>% 
  drop_na()
...
cw_statements <- labels_raw_tibble %>% 
  ...
  dplyr::select(variable_name, cw_statement)
...
main_act <- raw_data %>% 
  mutate(main_activity = case_when(
    ...
    mpl_105i=="Yes" ~ "Other", 
    TRUE~ "NA")) %>% 
  dplyr::select(main_activity) %>% 
  pull()

age_diff <- raw_data %>% 
  dplyr::select(marstat, aprcu0c, adfgrma0) %>% 
...
data_needed <- gss %>% dplyr::select(age,sex,education,
                              place_birth_canada,province,income_family,
                              citizenship_status)

# Keep all obeservations who are citizens and 18-year-old and over
citizens <- which(data_needed$citizenship_status == "By birth"|
                    data_needed$citizenship_status == "By naturalization")
data_needed <- data_needed[c(citizens),]
age_18andUp <- which(data_needed$age >= 18)
data_needed <- data_needed[c(age_18andUp),]

## re-assign variables
# education
data_needed$education<-ifelse(data_needed$education=="Less than high school diploma or its equivalent",
                              "Grade12&under",
                              ifelse(data_needed$education=="High school diploma or a high school equivalency certificate",
                                     "Highschool",
                                     ifelse(data_needed$education=="Trade certificate or diploma"|
                                              data_needed$education=="College, CEGEP or other non-university certificate or di...",
                                                   "College/Associate Degree",
                                                   ifelse(data_needed$education=="University certificate or diploma below the bachelor's level"|
                                                            data_needed$education=="Bachelor's degree (e.g. B.A., B.Sc., LL.B.)",
                                                          "Bachelor&under(uni)",
                                                          ifelse(data_needed$education=="University certificate, diploma or degree above the bach...",
                                                                 "AboveBachelor(uni)","NA")))))

# birth country
data_needed$place_birth_canada <-ifelse(data_needed$place_birth_canada=="Born in Canada",
                                     "Yes",
                                     ifelse(data_needed$place_birth_canada=="Born outside Canada", 
                                            "No", "NA"))


# family income
# level 1: 1~74,999
# level 2: 75,000~124,999
# level 3: more than 125,000
data_needed$income_family <- ifelse(data_needed$income_family == "Less than $25,000"|
                                      data_needed$income_family == "$25,000 to $49,999"|
                                      data_needed$income_family == "$50,000 to $74,999",
                                       "level 1",
                                       ifelse(data_needed$income_family == "$75,000 to $99,999"|
                                                data_needed$income_family == "$100,000 to $ 124,999",
                                              "level 2",
                                              ifelse(data_needed$income_family == "$125,000 and more",
                                                     "level 3", "NA")))


# age
data_needed$age <- ifelse(data_needed$age < 30, "18-29",
                                ifelse(data_needed$age >= 30 & data_needed$age < 40, "30-39",
                                       ifelse(data_needed$age >= 40 & data_needed$age < 50, "40-49",
                                              ifelse(data_needed$age >= 50 & data_needed$age < 60, "50-59",
                                                     ifelse(data_needed$age >= 60 & data_needed$age < 70, "60-69",
                                                            ifelse(data_needed$age >= 70 & data_needed$age < 80, "70-79", "80+"))))))

# rename variables
data_needed <- data_needed %>% 
  rename(birth_in_canada = place_birth_canada,
         income_family = income_family)

# variables we need for analysis and omit missing values
data_needed <- data_needed[,c(1,2,3,4,5,6)]
data_needed <- na.omit(data_needed)
na_obse <- which(data_needed$sex == "NA"|data_needed$age == "NA"|
                       data_needed$education == "NA"|data_needed$birth_in_canada == "NA"|
                       data_needed$province == "NA"|data_needed$income_family == "NA")
data_needed <- data_needed[-c(na_obse),]

# create cells
data_needed$cell <- paste(data_needed$province, data_needed$age,data_needed$education,
                          data_needed$income_family)

# Saving the survey/sample data as a csv file 

write_csv(data_needed, "GSS_2017_data.csv")
